...
├── project/
│   ├── backend/              # C# Backend
│   │   ├── Program.cs        # Main entry point & command handler
│   │   ├── DirectoryScanner.cs  # High-performance file system scanning
│   │   ├── EmbeddingEngine.cs   # ML embeddings with ONNX Runtime
│   │   ├── SQLiteStore.cs       # Database operations & persistence
│   │   ├── OrganizerEngine.cs   # File classification & categorization
│   │   ├── IPC.cs               # JSON-based frontend communication
│   │   ├── MetadataExtractor.cs # File metadata extraction
│   │   ├── ExclusionRules.cs    # System folder filtering
│   │   ├── FileIndexer.csproj   # Project configuration
│   │   └── models/              # ONNX model files
│   │       └── minilm.onnx      # (Download separately)
│   │
...
│           ├── StarBorder.jsx      # Animated border effect
│           ├── WindowControls.jsx  # Window controls (min/max/close)
│           ├── shims/              # React module shims
│           │   ├── react.js
│           │   └── react-dom.js
│           └── components/
│               ├── FolderCard.jsx      # Folder preview card
│               ├── FileCard.jsx        # File preview card
│               ├── OrganizeButton.jsx  # Organization action button
│               ├── GlassSurface.jsx    # Glassmorphism surface
│               ├── GlassIcons.jsx      # Navigation dock icons
│               ├── FileManager.jsx     # File management view
│               ├── ScrambledText.jsx   # Animated logo text
│               ├── ProfileCard.jsx     # User profile card (2D)
│               └── Settings.jsx        # Settings & profile page
│
└── README.md
```
...
- **MiniLM ONNX Model**: Download from [Hugging Face](https://huggingface.co/optimum/all-MiniLM-L6-v2/resolve/main/model.onnx)
  - **Important**: The file downloads as `model.onnx` - you must rename it to `minilm.onnx`
...
### 2. Download and Setup ML Model

1. Download the MiniLM ONNX model from the link above
2. **Rename** the downloaded file from `model.onnx` to `minilm.onnx`
3. Place it in the following directory:
   ```
   project/backend/models/minilm.onnx
   ```
   
> **Note**: The model file is already ignored in `.gitignore` to prevent it from being pushed to the repository.
